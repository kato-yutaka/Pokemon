...
CSV.foreach('db/pokedex_csv/skillname.csv') do |row|
  Skillname.create(:name => row[0], :type => row[1], :category => row[2], :power => row[3], :hit => row[4], :pp => row[5], :direct => row[6], :target => row[7], :eff => row[8])
end
CSV.foreach('db/pokedex_csv/type.csv') do |row|
  Type.create(:name => row[0])
end
CSV.foreach('db/pokedex_csv/type.csv') do |row|
  Type.create(:name => row[0])
end
CSV.foreach('db/pokedex_csv/tool.csv') do |row|
  Wepon.create(:name => row[0], :eff => row[1], :get => row[2])
end
CSV.foreach('db/pokedex_csv/cha.csv') do |row|
  Cha.create(:name => row[0], :ex => row[1])
end
CSV.foreach('db/pokedex_csv/egg.csv') do |row|
  Egg.create(:name => row[0])
end
CSV.foreach('db/pokedex_csv/per.csv') do |row|
...
#Pokedex.create(name: pokedex.name ,  hp: pokedex.tribeValue[0] , atk: pokedex.tribeValue[1] ,  def: pokedex.tribeValue[2] , satk: pokedex.tribeValue[3], sdef: pokedex.tribeValue[4] , spd: pokedex.tribeValue[5], pic_data: File.read(p) )
#end